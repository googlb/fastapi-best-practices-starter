...
class SysUserService:
    ...
    async def update_user(
        ...
        if obj_in.username and obj_in.username != user.username:
            existing_user = await crud_user.get_by_username(session, obj_in.username)
            if existing_user:
                return Result.error(400, "用户名已存在")

        # 如果更新邮箱，检查是否已存在
        if obj_in.email and obj_in.email != user.email:
            existing_user = await crud_user.get_by_email(session, obj_in.email)
            if existing_user:
                return Result.error(400, "邮箱已存在")

        # 更新用户
        user = await crud_user.update(session, user, obj_in)
        return Result.success(user)
