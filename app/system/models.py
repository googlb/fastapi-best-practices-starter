...
from app.db.mixins import BaseModel, SystemModel, TimestampMixin
...
class SysUserRole(TimestampMixin, SQLModel, table=True):
    ...
    user_id: int = Field(
        foreign_key="sys_users.id", primary_key=True, description="用户ID"
    )
    role_id: int = Field(
        foreign_key="sys_roles.id", primary_key=True, description="角色ID"
    )
...
class SysRoleMenu(TimestampMixin, SQLModel, table=True):
    ...
    menu_id: int = Field(
        foreign_key="sys_menus.id", primary_key=True, description="菜单ID"
    )
...
class SysUser(BaseModel, table=True):
    ...
    email: str | None = Field(
        default=None, unique=True, index=True, max_length=100, description="邮箱"
    )
...
class SysUserToken(SQLModel, table=True):
    ...
    # 关系反向引用
    user: SysUser | None = Relationship(back_populates="tokens")
...
class SysRole(SystemModel, table=True):
    ...
    description: str | None = Field(
        default=None, max_length=200, description="角色描述"
    )

    # 关系
    users: list[SysUser] = Relationship(back_populates="roles", link_model=SysUserRole)
    menus: list["SysMenu"] = Relationship(
        back_populates="roles", link_model=SysRoleMenu
    )
...
class SysMenu(SystemModel, table=True):
    ...
    parent_id: int | None = Field(
        default=None, foreign_key="sys_menus.id", description="父菜单ID"
    )
    title: str = Field(max_length=50, description="菜单标题")
    name: str | None = Field(default=None, max_length=50, description="路由名称")
    path: str | None = Field(default=None, max_length=200, description="路由路径")
    component: str | None = Field(
        default=None, max_length=200, description="组件路径"
    )
    icon: str | None = Field(default=None, max_length=50, description="图标")
    sort: int = Field(default=0, description="排序")
    permission: str | None = Field(
        default=None, max_length=100, index=True, description="权限标识"
    )
    ...
    parent: Optional["SysMenu"] = Relationship(
        back_populates="children", sa_relationship_kwargs={"remote_side": "SysMenu.id"}
    )
...
class SysDictData(SystemModel, table=True):
    ...
    dict_id: int = Field(
        foreign_key="sys_dicts.id", ondelete="CASCADE", description="字典ID"
    )
    ...
    class_name: str | None = Field(
        default=None, max_length=50, description="样式属性"
    )
